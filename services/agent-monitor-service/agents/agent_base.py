...
class IntelligentOrchestrator:
    ...
    async def execute_task(self, task):
        """Assign the task to an appropriate agent and execute."""
        best_agent = self.find_best_agent_for_task(task)
        if best_agent:
            print(f"Executing task {task.task_id} with agent {best_agent['agent_id']}")
            # Simulate async task execution
            await asyncio.sleep(random.uniform(0.5, 2))  # Mock execution time
            # Update the agent state after task completion
            best_agent['state'] = 'idle'
            print(f"Task {task.task_id} completed by agent {best_agent['agent_id']}")
        else:
